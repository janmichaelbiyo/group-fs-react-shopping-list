...
import { resetList, updateListPurchased } from '../../itemAPI/item.api';
...
function Footer({ itemData, shoppingListRefreshCall }) {
  ...
  return (
    <>
      <button onClick={(event) => handleClickClear(itemData)}>
        Clear Items
      </button>
      <button onClick={(event) => handleClickResetPurchased()}>
        Reset Buys
      </button>
    </>
  );
